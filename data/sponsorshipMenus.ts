import type { SponsorshipMenuData } from "@/lib/types";

/**
 * details 統一形式: [主な仕様, 提出期限 or 補足]（枠管理メニューは先頭行を自動生成）
 *
 * 募集状態の切替:
 * - recruitmentClosed: true で募集終了 / false または未設定で募集中
 * - recruitmentPriceLine + recruitmentSlotLabel で一覧の価格・枠行を自動表示
 * - 枠数変更時は recruitmentSlotLabel と detailText 内の表記を更新
 */
export const sponsorshipMenusData: SponsorshipMenuData[] = [
  // パンフレット広告
  {
    id: "pamphlet",
    title: "パンフレット広告",
    iconKey: "FileText",
    price: "¥5,000 〜 ¥60,000",
    category: "印刷物",
    recruitmentClosed: true,
    recruitmentPriceLine: "¥5,000〜¥60,000／1/8〜2ページ",
    desc: "当日配布のA5パンフレットに掲載させていただきます。協賛企業一覧への掲載も含みます。",
    details: [
      "白黒・カラー対応、数千部規模",
      "提出期限：2026年7月3日(金)",
    ],
    detailText: `技大祭当日に来場者へ配布される公式パンフレット（A5サイズ）に貴社広告を掲載させていただきます。

【協賛金額】
¥5,000〜¥60,000／掲載サイズにより異なります

【広告詳細】
・1/8ページから最大2ページまでお選びいただけます
※詳しくは技大祭協賛広告サイズ・金額表をご確認ください
・白黒・カラー両方に対応。協賛企業一覧ページへの貴社名掲載も含みます
・配布数は来場者数に応じ数千部規模を想定しています

【提出期限】
2026年7月3日(金)

※掲載イメージは昨年度のパンフレットです。`,
    pamphletImages: [
      "/brochure_cost_1.png",
      "/brochure_cost_2.png",
      "/brochure.jpg",
      "/brochure_mosaic.png",
    ],
    pamphletCouponText: `パンフレット広告をご応募いただいた企業様向けに、切り取り型クーポンの差し込みを無料でご用意しています（主に飲食店様を想定）。

特典内容・有効期限・店舗名などは貴社にてご検討いただき、券面デザインは実行委員会でご用意いたします。ご希望の場合は、パンフレット広告の応募・入稿時にお申し付けください。なお、クーポンの応募締め切りはパンフレット広告と同様の2026年7月3日(金)です。`,
    pamphletCouponImages: [
      { src: "/coupon_omote.png", label: "クーポン券面サンプル（表面）" },
      { src: "/coupon_ura.png", label: "クーポン券面サンプル（裏面）" },
    ],
  },
  // ユニフォーム広告
  {
    id: "uniform",
    title: "ユニフォーム広告",
    iconKey: "Shirt",
    price: "¥80,000 〜 ¥100,000",
    category: "アパレル",
    recruitmentClosed: true,
    desc: "約300名の実行委員が当日などに着用するTシャツにロゴを掲載させていただきます。袖・背中裾・背中首下の3タイプからお選びいただけます。",
    details: [
      "タイプA(袖)：¥100,000／先着2口",
      "タイプB(背中裾)：¥80,000／先着2口",
      "タイプC(背中首下)：¥100,000／先着1口",
      "提出期限：2026年7月17日(金)",
    ],
    detailText: `【ご案内】
  ・細い線(1mm以下)は滲む場合があります
  ・完成ユニフォーム(2着以上は有料)を後日送付いたします
  ・印刷データは実行委員会にて作成いたします`,
    uniformTypes: [
      {
        label: "タイプA（袖）",
        price: "¥100,000",
        limit: "先着2口",
        recruitmentClosed: true,
        detail: "袖部分にロゴおよび企業名を掲載いたします。",
        deadline: "2026年7月17日(金)",
      },
      {
        label: "タイプB（背中裾）",
        price: "¥80,000",
        limit: "先着2口",
        recruitmentClosed: true,
        detail: "背中裾部分にロゴおよび企業名を掲載いたします。",
        deadline: "2026年7月17日(金)",
      },
      {
        label: "タイプC（背中首下）",
        price: "¥100,000",
        limit: "先着1口",
        recruitmentClosed: true,
        detail: "背中首下部分にロゴおよび企業名を掲載いたします。",
        deadline: "2026年7月17日(金)",
      },
    ],
    uniformImages: ["/45th_uniform.png", "/44th_uniform.png", "/uniform_back.png"],
  },
  // ホームページ広告
  {
    id: "website",
    title: "ホームページ広告",
    iconKey: "Monitor",
    price: "¥15,000",
    category: "デジタル",
    recruitmentClosed: true,
    recruitmentPriceLine: "¥15,000／1口",
    desc: "協賛者様指定URLへのリンク付きバナーを、サイト内ほぼ全ページで5秒ごとにローテーション表示させていただきます。",
    details: [
      "横長バナー（縦横比2:3）・全ページローテーション",
      "提出期限：2026年8月28日(金)",
      "パンフレット1/2・1/8ページのデータ流用可",
    ],
    detailText: `技大祭公式ホームページのほぼ全ページに、5秒ごとに切り替わるローテーションバナーとして貴社広告を掲載させていただきます。
    
  【協賛金額】
  ¥15,000／1口

  【広告詳細】
  ・横長バナー（縦横比2:3）
  ・広告サイズは端末により異なります
  ・協賛者様指定URLへのリンク付きです
  ・パンフレット1/2・1/8ページのデータを流用いただけます
  ・掲載時期はデータの提出があり次第～年度末頃までです

  【提出期限】
  2026年8月28日(金)`,
    websiteImages: [
      {
        src: "/Website.png",
        label: "掲載イメージ（モバイル・全体）※昨年度のHPです",
      },
      { src: "/website_pc.png", label: "PC表示 ※昨年度のHPです" },
      {
        src: "/website_smartphone.jpg",
        label: "スマートフォン表示 ※昨年度のHPです",
      },
    ],
    websiteUrl: "https://www.nutfes.net/",
  },
  // のぼり広告
  {
    id: "nobori",
    title: "のぼり広告",
    iconKey: "Nobori",
    price: "¥10,000/本 〜 （10本セット ¥80,000）",
    category: "会場",
    recruitmentClosed: true,
    recruitmentPriceLine: "¥10,000/本〜（10本セット¥80,000）",
    desc: "技大祭当日の会場内に、貴社のロゴとお名前を掲載した縦型のぼりを来場者動線などPR効果の高い場所に設置いたします。のぼり広告のデザインは、企業様からご提供いただいたロゴ素材および掲載名称をもとに、技大祭実行委員会が制作いたします。企業様による完成データの入稿には対応しておりません。",
    details: [
      "600mm×1500mm、仮デザイン確認後制作",
      "提出期限：2026年7月31日(金)",
      "設置場所は委員会選定（昨年度：バス停歩道脇等）※強風時は撤去・変更の場合あり",
    ],
    detailText: `【協賛金額】
1本 ¥10,000
10本セット ¥80,000（2万円お得）
（例：15本＝8万×1＋1万×5＝13万円）

【広告詳細】
・サイズ：600mm×1500mm
・ロゴ：枠内最大配置。背景色は当委員会で選択（白/黒）
・企業名：(有)(株)に省略させていただきます。縦横書き選択可
・アルファベットは90度傾き（縦書き方向に回転した表記）となります
・制作：仮デザイン作成→ご確認→修正対応いたします

【設置・お渡し】
技大祭当日の会場内の来場者動線等PR効果の高い場所へ設置いたします（昨年度：バス停歩道脇等）。場所指定はいたしかねます。複数本は可能な限りまとめて配置いたします
※強風の場合、撤去または設置場所を変更する場合がございます
当日会場にお越しの場合は現物お持ち帰りいただけます（郵送返却は行っておりません）
※お持ち帰りされない場合は、技大祭終了後に破棄させていただきます

【提出期限】
2026年7月31日(金)

※のぼりデザインは昨年度のイメージです。`,
    noboriImages: ["/nobori.png", "/nobori_out.jpg"],
  },
  // ステージ・横断幕広告
  {
    id: "stage",
    title: "ステージ・横断幕",
    iconKey: "Flag",
    price: "¥200,000",
    category: "会場",
    recruitmentClosed: true,
    desc: "ステージバックボード・外会場の2種類をご用意しています。目玉イベント会場での高い露出が可能です。",
    details: [
      "ステージバックボード：¥200,000／募集終了・ゲストヨネダ2000出演",
      "外会場横断幕：¥200,000／募集終了・講義棟アナウンス特典付",
      "提出期限：2026年7月31日(金)",
    ],
    detailText: "",
    stageBannerTypes: [
      {
        title: "ステージバックボード広告協賛",
        desc: "今年の目玉ゲスト出演を予定する体育館ステージのバックボードに掲載させていただきます。音楽イベントや中夜祭など、来場者の注目が集まる場面での露出が可能です。",
        price: "一口 ¥200,000",
        limit: "先着4口",
        recruitmentClosed: true,
        size: "幅2.4m×高さ0.9m",
        deadline: "2026年7月31日(金)",
        guestHighlight: {
          label: "今年の目玉ゲスト",
          name: "ヨネダ2000",
          description:
            "第45回技大祭の体育館ステージでは、お笑いコンビ「ヨネダ2000」が出演予定です。ゲスト出演や中夜祭など、会場の熱気が高まる瞬間にバックボード広告が映え、来場者の記憶に残る露出機会となります。",
        },
        details: [
          "人物の顔写真などのステージ演出・雰囲気醸成の妨げとなる可能性のある画像の使用はご遠慮いただいております（推奨：企業名・ロゴ・キャッチコピー等）",
          "先着順に上部に配置いたします",
        ],
        images: ["/backboard_1.JPG", "/backboard_2.JPG", "/backboard_3.JPG"],
      },
      {
        title: "外会場横断幕広告協賛（新規）",
        desc: "外会場メインストリートに面した総研棟3階壁面に横断幕を設置いたします。",
        price: "一口 ¥200,000",
        limit: "先着2口",
        size: "幅4m×高さ1m",
        deadline: "2026年7月31日(金)",
        recruitmentClosed: true,
        perk: {
          title: "講義棟内30秒アナウンス特典",
          description:
            "協賛いただいた企業様のメッセージを、技大祭当日に講義棟内で30秒間流させていただきます。外会場での視覚的な露出に加え、来場者へ音声でも直接アプローチできる、本協賛ならではの特典です。",
        },
        details: [
          "ロゴ・企業名・キャッチコピー等を掲載させていただきます",
          "QRコード掲載はお断りしております",
          "※強風の場合、撤去または設置場所を変更する場合がございます",
        ],
        images: [
          "/out_banner.jpg",
          "/out_banner_map.png",
          "/out_banner_place.png",
        ],
      },
    ],
  },
  // うちわ広告
  {
    id: "uchiwa",
    title: "うちわ広告",
    iconKey: "Sun",
    price: "¥60,000 / 1,000枚",
    category: "会場",
    recruitmentClosed: true,
    recruitmentPriceLine: "¥60,000／1,000枚",
    recruitmentSlotLabel: "先着3口",
    desc: "協賛者様の広告をうちわ片面に印刷し、当日受付・イベント時等で来場者へ配布させていただきます。熱中症対策としても活用されます。",
    details: [
      "片面掲載、当日受付・イベント時配布",
      "提出期限：2026年7月31日(金)",
    ],
    detailText: `技大祭当日受付や各種イベント時等に来場者へ配布するうちわに、貴社広告を片面掲載させていただきます。猛暑時の熱中症対策としても活用されます。

【協賛金額】
一口 1,000枚 ¥60,000 先着3口

【広告詳細】
うちわの片面に広告を掲載させていただきます

【提出期限】
2026年7月31日(金)`,
    uchiwaImages: ["/fan.jpg"],
  },
  // 企業ブース協賛
  {
    id: "booth",
    title: "企業ブース協賛",
    iconKey: "Users",
    price: "¥60,000 / 1ブース",
    category: "会場",
    recruitmentClosed: true,
    recruitmentPriceLine: "¥60,000／1ブース",
    desc: "来場者に事業内容・製品・サービス等を直接ご説明いただき、地域・学生に業界の展望を知っていただける企画です。",
    details: [
      "両日 10:00〜17:00（9:30〜準備）講義棟1階",
      "求人目的ではご利用いただけません（インターンシップの告知も不可）",
    ],
    detailText: `来場者を対象に事業内容や製品・サービス等をご説明いただき、地域の方々や技大の学生に業界の展望を知っていただける企画「企業ブース」をご用意しています。

【協賛金額】
1ブース ¥60,000

【日時】
・2026年9月19日(土)・20日(日) 両日10:00〜17:00（9:30〜準備開始）
・お好きな日時でご説明いただけます

【場所】
〒940-2188 新潟県長岡市上富岡町1603-1 長岡技術科学大学 講義棟1階

【設営・準備】
・9/18(金)設営可。当日朝設営も可能です
・荷物送付：〒940-2127 新潟県長岡市新産1-2-3 ヤマト運輸 長岡新産１丁目営業所(041800) 止置き→実行委員が回収・設置いたします
・終了後は担当委員に送り状宛名と共に荷物をお渡しいただければ、着払いでお送りいたします

【開催形式】
・ブースは幅約3m×奥行き約3mをご用意しています。足りない場合はご相談ください
・必要に応じて机・椅子・電源(250W/ブース)・プロジェクター用スクリーン等をご用意いたします（プロジェクターは数に限りがあります。3m以上HDMIケーブルをご用意ください）

【注意事項】
・求人目的ではご利用いただけません。インターンシップの告知も不可とさせていただいております
・部屋外での配布・呼び込みはご遠慮いただいております。執拗な勧誘禁止。送迎バスは予定しておりません
※詳細は下記リンクよりご確認ください`,
    boothImages: [
      "/booth1.png",
      "/booth2.JPG",
      "/booth3.JPG",
      "/booth4.JPG",
    ],
    boothUrl:
      "https://www.nagaokaut.ac.jp/student/career/for-business/index.html",
  },
  // SNS広告
  {
    id: "sns",
    title: "SNS広告 (新規)",
    iconKey: "Instagram",
    price: "¥20,000",
    category: "デジタル",
    recruitmentPriceLine: "¥20,000",
    recruitmentSlotLabel: "先着9口",
    recruitmentClosed: true,
    desc: "技大祭公式Instagram(@nutfes)で貴社広告を投稿させていただきます。企画紹介と抱き合わせで、技大祭最終日までに1万回以上のインプレッションを保証いたします。",
    details: [
      "1万回以上インプレッション保証・2ページ構成",
      "提出期限：2026年7月17日(金)",
      "8月下旬順次投稿・永続アーカイブ",
    ],
    detailText: `約2,300人のフォロワーを持つInstagram技大祭公式アカウント(@nutfes)において、貴社広告を投稿させていただきます。

  【協賛金額】
  一口 ¥20,000 先着9口（技大祭最終日までに1万回以上インプレッション保証）

  【広告詳細】
  ・投稿形態：フィード投稿（1投稿2ページ構成）
  ・表紙ページ：実行委員会作成
  ・広告ページ：企業様デザイン（4:5縦長）
  ・本題ページ：企画内容紹介
  ・8月下旬順次投稿、永続アーカイブ

  【掲載期間】
  投稿後〜永続アーカイブ（削除なし）

  【提出期限】
  2026年7月17日(金)

  【制作・入稿】
  ・投稿内容は事前に審査・許諾いたします
  ・原則企業様にご用意いただきます。ご依頼があれば、実行委員会での制作も可能です（詳しくは実行委員会制作サンプルをご確認ください）`,
    snsImages: [
      { src: "/instagram.png", label: "投稿構成イメージ（表紙・広告・本題）" },
      { src: "/instagram_demo.png", label: "フィード投稿デモ" },
      {
        src: "/SNS_1.png",
        label: "実行委員会制作サンプル：1枚目（会社の概要）",
      },
      {
        src: "/SNS_2-1.png",
        label: "実行委員会制作サンプル：2枚目デザイン案1（会社の特色）",
      },
      {
        src: "/SNS_2-2.png",
        label: "実行委員会制作サンプル：2枚目デザイン案2（会社の特色）",
      },
      {
        src: "/SNS_2-3.png",
        label: "実行委員会制作サンプル：2枚目デザイン案3（会社の特色）",
      },
    ],
    snsUrl: "https://www.instagram.com/nutfes/",
  },
];
